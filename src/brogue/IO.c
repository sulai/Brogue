	...
	for (i=0; i<DCOLS; i++) {
		for(j=0; j<DROWS; j++) {
			...
			if (playerCanSeeOrSense(i, j)
				...
				&& (percentOfCells >= 100 || rand_range(1, 100) <= percentOfCells) || tileCatalog[tile].glowLight == TORCH_LIGHT) {
...
//                applyColorAverage(&cellBackColor, &memoryOverlay, 25);
                applyColorAverage(&cellBackColor, &blue, 20);
            }
...
// show stealth range
    if (rogue.displayAggroRangeMode) { // && (pmap[x][y].flags & IN_FIELD_OF_VIEW)
        ...
        if (cat_jR && distance <= rogue.aggroRange * 2) {
            applyColorAverage(&cellForeColor, &white, 50);
            applyColorAverage(&cellBackColor, &orange, 40);
        }
    }
    ...
    {
		if( cat_lt[x][y] == 2
            || (cat_lt[x][y] == -1 && displayDetail[x][y] == DV_LIT) )
		{
...
//abaraba
//test
//this caused saved game going out of sync sometimes
//            if(rogue.depthLevel < 11)
//                pmap[x][y].flags &= ~IS_IN_SHADOW;
//... perhaps need to recheck what was it here for in the 1st place...
// and can we actually do without it?

		}
		else
		if( cat_lt[x][y] == 1
            || (cat_lt[x][y] == -1 && displayDetail[x][y] == DV_DARK) )
		{
            applyColorAverage(&cellForeColor, &black, 40);
            applyColorAverage(&cellBackColor, &black, 70);

			applyColorAverage(&cellForeColor, &blue, 25);
        ...
        if( cat_lt[x][y] == 0
            || (cat_lt[x][y] == -1 && displayDetail[x][y] == DV_UNLIT) )
		{
                applyColorAverage(&cellForeColor, &black, 10);
                applyColorAverage(&cellBackColor, &black, 30);
                cat_lt[x][y] = 0;
        }
	}
	...
	if (needDistinctness && !rogue.trueColorMode) {
	...
	if (y < ROWS - 1) {
        ...
        if (monst == &player) {
            if (player.status[STATUS_INVISIBLE]) {
                ...
                encodeMessageColor(monstName, strlen(monstName) - 4, &playerInvisibleColor);
